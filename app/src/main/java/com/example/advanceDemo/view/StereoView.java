...
/**
 * Created by Mr_immortalZ on 2016/7/10. email : mr_immortalz@qq.com
 */
public class StereoView extends ViewGroup {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (yVelocity > standerSpeed
                        || ((getScrollY() + mHeight / 2) / mHeight < mStartScreen)) {
                    mState = State.ToPre;
                } else if (yVelocity < -standerSpeed
                        || ((getScrollY() + mHeight / 2) / mHeight > mStartScreen)) {
    ...
    private void toPreAction(float yVelocity) {
        ...
        addPre();// 增加新的页面
        // 计算松手后滑动的item个数
        int flingSpeedCount = (yVelocity - standerSpeed) > 0 ? (int) (yVelocity - standerSpeed)
                : 0;
    ...
    private void toNextAction(float yVelocity) {
        ...
        mState = State.ToNext;
        addNext();
        int flingSpeedCount = (Math.abs(yVelocity) - standerSpeed) > 0 ? (int) (Math
                .abs(yVelocity) - standerSpeed) : 0;
        addCount = flingSpeedCount / flingSpeed + 1;
        startY = getScrollY() - mHeight;
        setScrollY(startY);
    ...
    public void computeScroll() {
        ...
        if (mScroller.computeScrollOffset()) {
            if (mState == State.ToPre) {
                scrollTo(mScroller.getCurrX(), mScroller.getCurrY() + mHeight
                        * alreadyAdd);
    ...
    private void drawScreen(Canvas canvas, int i, long drawingTime) {
        ...
        float centerY = (getScrollY() > curScreenY) ? curScreenY + mHeight
                : curScreenY;
    ...
    public StereoView setItem(int itemId) {
        ...
        // LogUtil.m("之后curScreen " + mCurScreen + " getScrollY " +
        // getScrollY());
        return this;
    }
