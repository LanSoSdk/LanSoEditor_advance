...
public class DrawPadView extends FrameLayout {
    ...
    private void setTextureView() {
        ...
        mTextureRenderView.setDisplayRatio(AR_ASPECT_FIT_PARENT);
    ...
    public boolean startDrawPad(boolean pauseRecord) {
        ...
        if (mSurfaceTexture != null && drawPadWidth > 0 && drawPadHeight > 0) {
            renderer = new DrawPadViewRunnable(getContext(), drawPadWidth,drawPadHeight);
                ...
                if (isCheckPadSize) {
                    encWidth = make16Multi(encWidth); // 编码默认16字节对齐.
                    encHeight = make16Multi(encHeight);
                }
                if (isCheckBitRate || encBitRate == 0) {
                    encBitRate = checkSuggestBitRate(encHeight
                            * encWidth, encBitRate);
                }
    ...
    public MVLayer addMVLayer(LSOAsset asset) {
    ...
    public MVLayer2 addMVLayer2(LSOMvAsset2 asset) throws Exception{
        if (renderer != null && asset!=null)
            ...
            LSOLog.e("addMVLayer error render ==null");
            return null;
        }
    }
    /**
